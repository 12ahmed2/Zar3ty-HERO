...
// ✅ FIXED: Checkout button now redirects to /checkout page instead of calling API directly
if (e.target.closest && e.target.closest('#btn-checkout')) {
if (typeof window.me === 'undefined' || !window.me) { 
  toast(detectLanguage('Log in to checkout') === LANG? 'Log in to checkout': await translate('Log in to checkout', detectLanguage('Log in to checkout'), LANG)); 
  show(document.querySelector('#dlg-login')); 
  return; 
}
// ✅ Redirect to checkout page to collect address/phone
...
/* 1. TRANSLATION CACHE & PRODUCTION-READY FALLBACK */
const TRANSLATION_CACHE = new Map();
async function translateCached(text, src, dest) {
...
// Patch existing translate calls to use cache automatically when available
window.safeTranslate = translateCached;

/* 2. DEBOUNCED SEARCH (Fixes duplicate event listener issue) */
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (searchInput) {
    ...
    newInput.addEventListener('input', debounce((e) => searchProducts(e.target.value), 250));
  }
});

/* 3. OPTIMISTIC CART UI & AUTO-REFRESH SYNC */
// Silent cart sync every 30s
setInterval(async () => {
  if (document.hidden) return;
  try { await updateCartBadge(); } catch {}
}, 30000);

// Instant cart badge update on add (Optimistic)
const _origAddToCart = window.addToCart || addToCart;
window.addToCart = async function(productId) {
  // Optimistic UI bump
  const navBadge = document.querySelector('#cart-count');
  if (navBadge) navBadge.textContent = String(parseInt(navBadge.textContent || 0) + 1);
  
  try {
    return await _origAddToCart(productId);
  } catch (err) {
    // Revert on failure
    if (navBadge) navBadge.textContent = String(Math.max(0, parseInt(navBadge.textContent) - 1));
    throw err;
  }
};

/* 4. CHECKOUT FLOW UPGRADE (Redirects to /checkout page) */
// ✅ This enhancement is now integrated into the main checkout handler above
// The duplicate listener has been removed to prevent conflicts

/* 5. LANGUAGE SYNC & RTL ENHANCEMENT */
window.syncLanguageFromStorage = () => {
...
/* 6. LOADING SKELETONS & EMPTY STATE POLISH */
const _origRenderProductPage = renderProductPage;
window.renderProductPage = async function() {
  if (!els.grid) return _origRenderProductPage();
  
  // Inject skeleton if empty
  if (!productsCache.length) {
    els.grid.innerHTML = Array(9).fill(`<div class="card skeleton" style="height:250px;"></div>`).join('');
  }
  return _origRenderProductPage();
};

// Add skeleton animation to head if missing
if (!document.getElementById('skeleton-styles')) {
  const style = document.createElement('style');
  style.id = 'skeleton-styles';
  style.textContent = `@keyframes pulse { 0%{opacity:0.6} 50%{opacity:1} 100%{opacity:0.6} } .skeleton { background: linear-gradient(90deg,#f0f0f0 25%,#e8e8e8 50%,#f0f0f0 75%); background-size: 200% 100%; }`;
...
if ('ontouchstart' in window) {
  ...
  style.textContent = `
    @media (hover: none) {
      .btn, .nav-link, .reaction-btn, .icon-btn {
        min-height: 44px;
        min-width: 44px;
        padding: 12px 16px;
      }
...
/* 10. EXPORT ENHANCED FUNCTIONS FOR EXTERNAL USE */
window.index = {
  translateCached,
  debounce,
  syncLanguageFromStorage,
  renderProductPage: window.renderProductPage,
  searchProducts,
  loadProducts,
  renderCart,
  updateCartBadge
